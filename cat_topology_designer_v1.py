...
# --- PAGE CONFIG ---
st.set_page_config(page_title="CAT Topology v12.0 (Hyperscale Logic)", page_icon="⚡", layout="wide")
...
<style>
    ...
    .kpi-card { background-color: #f8f9fa; padding: 10px; border-radius: 5px; text-align: center; border-left: 5px solid #6c757d; box-shadow: 1px 1px 3px rgba(0,0,0,0.1); }
...
# ==============================================================================
# 2. SOLVER (HYPERSCALE LOGIC v12)
# ==============================================================================

def solve_topology_v12(inputs):
    ...
    # --- INTELLIGENT VOLTAGE SELECTION (HYPERSCALE LOGIC) ---
    forced_msg = ""
    if inputs['volts_mode'] == 'Manual':
        kv_list = [inputs['volts_kv']]
    else:
        # Standard Tiers
        # Logic: Don't even try 13.8kV if load is massive. It wastes computing time and gives bad designs.
        if p_gross > 200:
            kv_list = [69.0, 115.0]
            forced_msg = "Load > 200MW: Starting at 69kV (Transmission Level)."
        elif p_gross > 40:
            kv_list = [34.5, 69.0]
            forced_msg = "Load > 40MW: Starting at 34.5kV to reduce copper losses and SC levels."
        elif p_gross > 5:
            kv_list = [13.8, 34.5]
        else:
            kv_list = [4.16, 13.8]

    res['voltage_msg'] = forced_msg
    final_sol = None
...
# ==============================================================================
# 3. UI INPUTS (PRESERVED)
# ==============================================================================

if 'inputs_v12' not in st.session_state:
    st.session_state['inputs_v12'] = {
...
def get(k): return st.session_state['inputs_v12'].get(k)
def set_k(k, v): st.session_state['inputs_v12'][k] = v

with st.sidebar:
    st.title("Inputs v12.0")
...
res = solve_topology_v12(st.session_state['inputs_v12'])

# ==============================================================================
# 4. DASHBOARD
# ==============================================================================

st.title("CAT Topology Designer v12.0")
st.caption("Integrated: Physics + BESS + ANSI Diagrams + Hyperscale Logic")
...
if res['pass']:
    ...
    # MESSAGES
    if res['voltage_msg']:
        st.markdown(f'<div class="info-box">ℹ️ {res["voltage_msg"]}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="success-box">✅ <b>Optimized:</b> Configuration valid at <b>{sol["kv"]} kV</b> with {sol["n_buses"]} Iso-Parallel Buses.</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="error-box">❌ <b>Optimization Failed:</b> No valid configuration found in any voltage tier.</div>', unsafe_allow_html=True)
    best = res['diag']['best_attempt']
    if best:
        st.write(f"**Closest Attempt at {best['kv']} kV:**")
        st.warning(f"Reasons: {', '.join(best['reasons'])}")
